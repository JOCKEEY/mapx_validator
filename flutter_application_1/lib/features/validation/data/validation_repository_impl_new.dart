...
class ValidationRepositoryImpl implements ValidationRepository {
    ...
    try {
      ...
      await database.insertValidation(
        ValidationsCompanion(
          id: drift.Value(validation.id),
          parcelId: drift.Value(validation.parcelId),
          status: drift.Value(validation.status),
          remarks: drift.Value(validation.remarks),
          latitude: drift.Value(validation.latitude),
          longitude: drift.Value(validation.longitude),
          syncStatus: drift.Value(validation.syncStatus),
          createdAt: drift.Value(validation.createdAt),
          updatedAt: drift.Value(validation.updatedAt),
          retryCount: drift.Value(validation.retryCount),
        ),
      );
  ...
  Future<Either<Failure, List<ValidationEntity>>> getValidationsByParcelId({
    ...
    try {
      ...
      return Left(ValidationFailure('Failed to get validations: ${e.toString()}'));
    }
  }
  ...
  Future<Either<Failure, List<ValidationEntity>>> getPendingValidations() async {
    try {
      ...
      return Left(ValidationFailure('Failed to get pending validations: ${e.toString()}'));
    }
  }
    ...
    try {
      ...
      await database.insertValidationPhoto(
        ValidationPhotosCompanion(
          id: drift.Value(photoId),
          validationId: drift.Value(validationId),
          localPath: drift.Value(localPath),
          fileName: drift.Value(localPath.split('/').last),
